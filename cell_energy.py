...
def input_fields():
    ...
    with col2:
        ...
        input_dict["p_firstloss"] = st.number_input("first cyc. loss %", value=10)
    ...
    with col3:
        ...
        input_dict["s_bulkdensity"] = st.number_input("bulk density, g/cc", value=0.855)
        st.markdown("---")
    ...
    with col4:
        ...
        input_dict["n_massfrac"] = st.number_input("mass fraction", value=0.954)
        ...
        input_dict["n_firstloss"] = st.number_input("first cyc. loss %", value=8)

    with col5:
        st.markdown("### current coll.")
        input_dict["p_cc_thickness"] = st.number_input("+ve thickness / µm", value=15)
        st.markdown("---")
        ...
        input_dict["n_cc_thickness"] = st.number_input("-ve thickness / µm", value=8)
...
def print_results(calculated_data):
    ...
    st.write(
        ...
        n/p ratio: \
            {calculated_data['n']['areal_cap'] / calculated_data['p']['areal_cap']:.1f}"
    )

    st.plotly_chart(plot_mass(), use_container_width=True)
    st.plotly_chart(plot_cross_section())
...
st.markdown(
    ...
    calculations. The default values for the positive and negative electrodes
    are based on the suggested baseline data for NMC532 vs graphite presented
    by Harlow et al, J. Electrochem. Soc. 166 (13) A3031-A3044 (2019).
...
celltype = st.radio("type", ("18650", "21700"), label_visibility="collapsed")
